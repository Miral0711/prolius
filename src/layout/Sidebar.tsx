          ...
          {navConfig.map((item) => {
            ...
            return (
              ...
              >
                ...
                >
                  <Icon className={cn("h-5 w-5 shrink-0", active ? "text-blue-400" : "text-white/60 group-hover:text-white")} />
                  {!collapsed && (
                ...
                <AnimatePresence>
                  {hoveredItem === item.label && hasChildren && (
                    <motion.div
                      ...
                      style={{
                        left: collapsed ? SIDEBAR_WIDTH_COLLAPSED + 12 : SIDEBAR_WIDTH_EXPANDED + 8,
                        top: mousePos.top
                      }}
                    >
                      {item.children?.map((sub) => {
                        ...
                        return (
                          <div
                            ...
                            onMouseEnter={(e) => handleSubMouseEnter(e, sub.label)}
                          >
                            <Link
                              to={sub.path || '#'}
                              ...
                              className={cn(
                                ...
                                subActive ? "bg-white/10 text-white font-medium" : "text-white/60 hover:bg-white/5 hover:text-white"
                              )}
                            >
                              <span>{sub.label}</span>
                              {subHasChildren && <ChevronRight className="h-3 w-3 opacity-50" />}
                            </Link>

                            {/* Level 2 Submenu */}
                            <AnimatePresence>
                              {hoveredSubItem === sub.label && subHasChildren && (
                                <motion.div
                                  initial={{ opacity: 0, x: -10 }}
                                  animate={{ opacity: 1, x: 0 }}
                                  exit={{ opacity: 0, x: -10 }}
                                  ...
                                  {sub.children?.map((leaf) => (
                                    <Link
                                      ...
                                      className={cn(
                                        ...
                                        isActive(leaf.path) ? "bg-white/10 text-white font-medium" : "text-white/60 hover:bg-white/5 hover:text-white"
                                      )}
                                    >
                                      {leaf.label}
                                    </Link>
