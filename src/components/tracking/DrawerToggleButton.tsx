...
export function DrawerToggleButton({ onClick, isOpen, side, className }: DrawerToggleButtonProps) {
  ...
  return (
    <Button
      onClick={onClick}
      className={cn(
        "absolute top-[68px] h-12 w-10 flex flex-col items-center justify-center gap-1 bg-white/90 backdrop-blur-xl border border-white/40 shadow-[0_8px_25px_rgba(0,0,0,0.08)] text-blue-600 hover:bg-white transition-all group z-[35]",
        isLeft 
    ...
    >
      <div className={cn(
        ...
        isLeft ? "bg-blue-50 text-blue-600" : "bg-indigo-50 text-indigo-600"
      )}>
        {isLeft ? <Bus className="h-4 w-4" /> : <Info className="h-4 w-4" />}
      </div>
      {isLeft ? (
        <ChevronRight className="h-3 w-3 text-slate-300 group-hover:text-blue-400 group-hover:translate-x-0.5 transition-all" />
      ) : (
