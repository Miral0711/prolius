...
export function CompactSummaryWidget({
  ...
  return (
    <DashboardCard className="!p-0 border-white/40 shadow-sm flex-1 flex flex-col overflow-hidden">
      <CardHeader title={title} icon={Icon} iconClassName={iconColorClass} />
      <div
      ...
      >
        {listData?.map((s) => (
          ...
          >
            ...
            <span className={`text-[11px] font-medium leading-none ${s.textClass}`}>{s.value}</span>
          </div>
        ))}
        {statData?.map((s, index) => (
          ...
          >
            <span className="text-xs font-normal text-slate-400 uppercase tracking-[0.02rem]">{s.label}</span>
            <div className="flex items-baseline gap-1.5">
              <span className={`text-xl font-medium leading-none ${s.colorClass}`}>{s.value}</span>
              <span className="text-xs font-normal text-slate-500">{s.sub}</span>
            </div>
          </div>
        ))}
      </div>
