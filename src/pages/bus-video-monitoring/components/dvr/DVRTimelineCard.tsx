    ...
    <div className="rounded-xl border border-white bg-white shadow-[0_2px_12px_-4px_rgba(0,0,0,0.08)] p-3">
      <div className="flex items-center justify-between gap-3 mb-3">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-[10px] font-semibold text-slate-700 uppercase tracking-widest leading-none">Temporal Analysis Chronology</h2>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2.5">
            {[
              ...
              <div key={l.label} className="flex items-center gap-1">
                ...
                <span className="text-[8px] font-medium text-slate-400 uppercase tracking-tight">{l.label}</span>
              </div>
            ))}
          </div>
          ...
          <div className="flex items-center gap-1 rounded-lg border border-slate-200 bg-slate-50 p-0.5 shadow-inner">
            <button type="button" className="h-5 w-5 flex items-center justify-center rounded text-[12px] font-semibold text-slate-400 hover:bg-white hover:text-slate-600 transition-all">−</button>
            <button type="button" className="h-5 w-5 flex items-center justify-center rounded text-[12px] font-semibold text-slate-400 hover:bg-white hover:text-slate-600 transition-all">+</button>
          </div>
        </div>
      </div>
      ...
      <div className="relative px-0.5">
        {/* Time Labels */}
        <div className="flex justify-between text-[8px] text-slate-400 font-mono font-medium mb-1.5 opacity-60">
          {hours.map(h => (
        ...
        >
              ...
              {hoveredEvent === i && (
                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2.5 py-1.5 bg-slate-900 border border-white/10 text-white rounded-xl shadow-2xl whitespace-nowrap z-50 animate-in fade-in zoom-in duration-200">
                  <div className="flex flex-col gap-0.5">
                    <span className="text-[9px] font-semibold text-blue-400 uppercase tracking-widest">{ev.type}</span>
                    <span className="text-[11px] font-mono font-semibold">{ev.time}</span>
                  </div>
          ...
          <div className="absolute top-0 bottom-0 w-0.5 bg-blue-600 z-20 shadow-[0_0_12px_rgba(37,99,235,0.6)]" style={{ left: `${progress}%` }}>
            <div className="absolute -top-2.5 left-1/2 -translate-x-1/2 bg-blue-600 text-white text-[8px] font-mono font-semibold px-1 py-0.5 rounded-full shadow-lg">
              {progress}%
            </div>
      ...
      <div className="flex flex-wrap items-center gap-2 mt-3">
        {[
          ...
          <span key={s.label} className={cn('inline-flex items-center gap-1.5 rounded-xl px-2.5 py-1 text-[9px] font-semibold border uppercase tracking-tight', s.c)}>
