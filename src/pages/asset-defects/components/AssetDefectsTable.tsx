    ...
    <DataTable className={cn('min-h-0', className)}>
      ...
      <DataTableBodyScroll>
        <DataTableTable className="min-w-[1305px]">
          <colgroup>
            {[118,105,100,115,220,115,120,118,130,64].map((w, i) => <col key={i} style={{ width: `${w}px` }} />)}
          </colgroup>
          <TableHeader>
            <tr>
            ...
            {rows.length === 0 ? (
              ...
              rows.map((row) => (
                <TableRow key={row.id} className={cn(row.archived && 'opacity-60')}>
                  ...
                  <TableCell><span className="font-mono text-2sm font-semibold tabular-nums text-[#2e5f8a]">{row.assetNumber}</span></TableCell>
                  ...
                  <TableCell><AssetDefectStatusBadge status={row.defectStatus} /></TableCell>
