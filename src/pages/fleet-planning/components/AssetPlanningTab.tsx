...
import { ASSET_ROWS, type AssetRow } from '../planning-table-data';
import { hasAssetConflict } from '../conflict-detection';
...
const EVENT_OPTIONS = [
  ...
  { value: 'corrosionInspection', label: 'Corrosion Inspection' },
  { value: 'electricalInspection', label: 'Electrical Inspection' },
  { value: 'ladenBrakeTest', label: 'Laden Brake Test' },
  { value: 'lolerTest', label: 'LOLER Test' },
];
...
const COLUMNS: ColumnDef<AssetRow>[] = [
  ...
  { key: 'corrosionInspection',  header: 'Corrosion Inspection',  width: '160px', render: (r) => <DateCell date={r.corrosionInspection} /> },
  { key: 'electricalInspection', header: 'Electrical Inspection', width: '160px', render: (r) => <DateCell date={r.electricalInspection} /> },
  { key: 'ladenBrakeTest',       header: 'Laden Brake Test',      width: '140px', render: (r) => <DateCell date={r.ladenBrakeTest} /> },
  { key: 'lolerTest',            header: 'LOLER Test',            width: '120px', render: (r) => <DateCell date={r.lolerTest} /> },
];
...
export function AssetPlanningTab() {
  ...
  const filtered = useMemo(() => {
    return ASSET_ROWS.filter((r) => {
      ...
      if (appliedSearch) {
        ...
        if (!r.assetNumber.toLowerCase().includes(q) && !r.location.toLowerCase().includes(q)) return false;
      }
      return true;
    });
  ...
  const conflictCount = filtered.filter(hasAssetConflict).length;
