...
/** Compact select — label stacked above input */
...
/** Compact text input — label stacked above input */
function FieldInput({
...
export default function AssetProfileEditPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const profile = ASSET_PROFILE_DETAIL_MOCK;

  const [form, setForm] = useState({
    profileStatus: profile.profileStatus as string,
    type: profile.type,
    category: profile.category,
    subCategory: profile.subCategory,
    odometerSetting: profile.odometerSetting,
    manufacturer: profile.manufacturer,
    model: profile.model,
    fuelType: profile.fuelType,
    engineType: profile.engineType,
    co2gkm: profile.co2gkm,
    adrTestInterval: profile.adrTestInterval,
    corrosionInspectionInterval: profile.corrosionInspectionInterval,
    ladenBrakeTestInterval: profile.ladenBrakeTestInterval,
    majorWheelServiceInterval: profile.majorWheelServiceInterval,
    pmiInterval: profile.pmiInterval,
    rubberIntegrityTestInterval: profile.rubberIntegrityTestInterval,
    serviceFrequency: profile.serviceFrequency,
    tankTestInterval: profile.tankTestInterval,
  });

  const set = (key: keyof typeof form) => (v: string) => setForm((f) => ({ ...f, [key]: v }));
  const detailPath = `/asset-management/profiles/${id ?? profile.id}`;

  // Split defects into 2 balanced columns
          ...
          <div className="flex flex-col gap-2 pb-2">
            ...
            <Breadcrumb>
              <BreadcrumbList>
                ...
                <BreadcrumbItem>
                  <BreadcrumbLink asChild>
                    <Link to={detailPath} className="text-2sm text-[#3d6b8e] hover:underline">Details</Link>
                  </BreadcrumbLink>
                </BreadcrumbItem>
                <BreadcrumbSeparator />
                <BreadcrumbItem>
                  <BreadcrumbPage className="text-2sm text-slate-600">Edit</BreadcrumbPage>
                </BreadcrumbItem>
              </BreadcrumbList>
            </Breadcrumb>
            ...
            <div className="grid grid-cols-1 gap-2 lg:grid-cols-[1fr_260px]">
              ...
              <PanelCard noPadding>
                <div className="flex items-center justify-between border-b border-slate-100 px-3 py-2">
                  <h3 className="text-[11px] font-bold uppercase tracking-[0.8px] text-[#1e3448]">Edit Asset Profile</h3>
                </div>

                <div className="p-3">
                  {/* 2-column field grid */}
                  <div className="grid grid-cols-2 gap-x-4 gap-y-3">
                    <FieldSelect label="Profile status" value={form.profileStatus} onChange={set('profileStatus')} options={STATUS_OPTIONS} required />
                    <FieldInput  label="Type"           value={form.type}          onChange={set('type')}          required />

                    <FieldSelect label="Category"     value={form.category}     onChange={set('category')}     options={CATEGORY_OPTIONS}     required />
                    <FieldSelect label="Sub category" value={form.subCategory}  onChange={set('subCategory')}  options={SUB_CATEGORY_OPTIONS} required />

                    <FieldInput  label="Manufacturer" value={form.manufacturer} onChange={set('manufacturer')} required />
                    <FieldInput  label="Model"        value={form.model}        onChange={set('model')}        required />

                    <FieldSelect label="Fuel type"    value={form.fuelType}    onChange={set('fuelType')}    options={FUEL_OPTIONS} />
                    <FieldSelect label="Engine type"  value={form.engineType}  onChange={set('engineType')}  options={ENGINE_OPTIONS} />

                    <FieldInput  label="CO2 (g/km)"        value={form.co2gkm}          onChange={set('co2gkm')} />
                    <FieldSelect label="Odometer setting"  value={form.odometerSetting} onChange={set('odometerSetting')} options={ODOMETER_OPTIONS} required />

                    <FieldSelect label="ADR test interval"          value={form.adrTestInterval}          onChange={set('adrTestInterval')}          options={INTERVAL_OPTIONS} />
                    <FieldSelect label="Corrosion inspection"       value={form.corrosionInspectionInterval} onChange={set('corrosionInspectionInterval')} options={INTERVAL_OPTIONS} />

                    <FieldSelect label="Laden brake test"           value={form.ladenBrakeTestInterval}   onChange={set('ladenBrakeTestInterval')}   options={INTERVAL_OPTIONS} />
                    <FieldSelect label="Major wheel service"        value={form.majorWheelServiceInterval} onChange={set('majorWheelServiceInterval')} options={INTERVAL_OPTIONS} />

                    <FieldInput  label="PMI interval"               value={form.pmiInterval}              onChange={set('pmiInterval')} />
                    <FieldSelect label="Rubber integrity test"      value={form.rubberIntegrityTestInterval} onChange={set('rubberIntegrityTestInterval')} options={['none', ...INTERVAL_OPTIONS]} />

                    <FieldSelect label="Service frequency"          value={form.serviceFrequency}         onChange={set('serviceFrequency')}         options={INTERVAL_OPTIONS} required />
                    <FieldSelect label="Tank test interval"         value={form.tankTestInterval}         onChange={set('tankTestInterval')}         options={INTERVAL_OPTIONS} />
                  </div>
                </div>

                {/* Sticky-style footer buttons */}
              ...
              <div className="flex flex-col gap-2">
                ...
                {/* Images 2×2 */}
                <PanelCard title="Edit Asset Profile Images">
